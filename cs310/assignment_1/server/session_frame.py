...
from a1.utils import save_attendance
from a1.utils.file_upload import handle_file_upload

class ScrollableFrame(ttk.Frame):
    def __init__(self, container, *args, **kwargs):
        super().__init__(container, *args, **kwargs)

        # Theme colors - use consistent colors throughout
        ...
        # Create main wrapper with 2 columns of equal width
        self.main_frame = ttk.Frame(self, style="Login.TFrame")
        self.main_frame.grid(row=0, column=0, sticky="nsew")
        self.rowconfigure(0, weight=1)
        self.columnconfigure(0, weight=1)

        # Make both columns equal width (weight=1)
    ...
    def show_chat(self):
        ...
        # Create a window in the canvas to hold scrollable content
        ...
        # Make sure the chat frame resizes properly
    ...
    def text_message_frame(self):
        ...
        # Path to icons for uploading and sending
        CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
        BASE_DIR = os.path.abspath(os.path.join(CURRENT_DIR, '..', '..'))
        UPLOAD_ICON_PATH = os.path.join(BASE_DIR, 'a1', 'public', 'upload.png')
        SEND_TEXT_ICON_PATH = os.path.join(BASE_DIR, 'a1', 'public', 'send-horizontal.png')

        # Load the images for the icons
        self.upload_icon = PhotoImage(file=UPLOAD_ICON_PATH)
        self.send_icon = PhotoImage(file=SEND_TEXT_ICON_PATH)

        # File share button with upload icon and consistent style
        self.file_share_btn = tk.Button(
            text_frame,
        ...
        self.file_share_btn.grid(column=1, row=0, pady=2, padx=2)
    ...
    def upload_file(self):
        ...
        send_chat(Chat(chat_type=ChatType.FILE, text=file_path, from_user=global_app_state.user, to_user=DUMMY_USER))
    ...
    def show_user_list(self):
        # --- Left: User list with consistent styling
        ...
        # Canvas and Scrollbar with consistent background
...
class SessionWindow(tk.Toplevel):
    def __init__(self, container, show_login=None):
        ...
        # main wrapper with theme and no borders or padding
        ...
        self.scroll.grid(row=1, column=0, columnspan=2, sticky="nsew", padx=0, pady=0)
    ...
    def show_header(self):
        style = ttk.Style()
        style.configure("Header.TLabel", font=("Segoe UI", 18, "bold"), background=self.bg_color, foreground="#3b82f6")
        style.configure("Session.TFrame", background=self.bg_color)
        style.configure("TButton", font=("Segoe UI", 12), background="#3b82f6", foreground="white")
        style.map("TButton", background=[("active", "#2563eb")])
        style.configure("Info.TLabel", font=("Segoe UI", 12), background=self.bg_color)
        ...
        # attendance info frame (no borders)
    ...
    def handle_session_end(self):
        ...
        tmp_file = save_attendance(global_app_state.attendance_list, session_id=uuid.uuid4())
        global_app_state.attendance_list_path = tmp_file
    ...
    def open_p2p_chat(self, user):
        if user.id != global_app_state.user.id:
            print(f"opening chat with {user.name}")
    ...
    def handle_chat(self):
        diff = global_app_state.chat - self.chats
        self.chats = global_app_state.chat.copy()

        if len(diff) == 0:
            return

        sorted_diff = sorted(diff, key=lambda chat: chat.timestamp)

        for i, chat in enumerate(sorted_diff):
            ...
            ttk.Label(frame, text=timestamp_str, style="MutedTime.TLabel").grid(row=0, column=1, sticky="e")
    ...
    def update(self):
        ...
        self.users = global_app_state.attendance_list.copy()
